...
config_to_save['func'] = None # set func to None as it's not directly representable in a simple text file
with open(f'results/config_{run_name}.txt', 'w') as f:
    for key, value in config_to_save.items():
        f.write(f"{key} : {value}\n")

data_to_save = {
    'losses': np.array(result_modadd['losses']),
    'losses_val': np.array(result_modadd['losses_val']),
    'accs': np.array(result_modadd['accs']),
    'accs_val': np.array(result_modadd['accs_val']),
...
np.savez(f'results/stats_{run_name}.npz', **data_to_save)